    ...
    <el-row>
      <el-col :span="24" :offset="0">
        <div style="text-align: center">
          <span>选择的可疑肺结节：</span>
          <span v-if="checkList.length!==0">{{printList(checkList)}}</span>
          <span v-if="checkList.length===0">还未选择可疑的肺结节！</span>
          <el-button
            type="primary"
      ...
      <el-row :gutter="20">
        <el-col :span="4" v-for="imageCode in allImageList" style="margin-top: 3em">
          <el-checkbox-group v-model="checkList">
            <div
              style="padding: 4px;"
              v-if="checkList.indexOf(allImageList.indexOf(imageCode))===-1"
            >
              <el-checkbox class="demonstration" :label="allImageList.indexOf(imageCode)">
                <div>编号： {{allImageList.indexOf(imageCode)}}</div>
              </el-checkbox>
              <img
                v-image-preview
                :src="formatImgSrc(imageCode)"
                style="width: 14vw; height: 14vw;margin-left: auto;margin-right: auto"
              >
            </div>
            <div
              style="padding: 4px; background-color: #e6a23c"
              v-if="checkList.indexOf(allImageList.indexOf(imageCode))>-1"
            >
              <el-checkbox class="demonstration" :label="allImageList.indexOf(imageCode)">
                <div>编号： {{allImageList.indexOf(imageCode)}}</div>
...
export default {
  ...
  methods: {
    ...
    subImg() {
      sessionStorage.setItem("img_list", JSON.stringify(this.checkList));
