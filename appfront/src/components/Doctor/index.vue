        ...
        >
          ...
          <el-table-column label="操作" min-width="200">
            <template scope="scope">
              ...
              <el-button
                v-if="getCTStatus(scope.$index, scope.row)===5"
              >检测中
              </el-button>
              <el-button
  ...
  export default {
    ...
    methods: {
      ...
      gotoDetect(index, row) {
        ...
        // set button to running
        row.status='RUNNING'
        setTimeout(() => {
          this.queryStatus(row)
          console.log('set up a timer to query detect status')
        }, 8000)
      ...
      queryStatus(row){
        ...
        allService.queryDetectStatus(params, (isok, data) => {
          if (isok) {
            if (data.data === 1) {
              this.timer = null
              row.status=''
