...
import math
import random
import livePredict_util
from config_livePredict import config_livePredict
from data_reader_livePredict import DATA_READER
...
def test():
    ...
    ckpt_path = "./ckpt_live/"
    data_path, step = livePredict_util.load_weight(ckpt_path)
    if step:
    ...
    print("corr_live:", float(corr_live) / float(live_num))
    print("corr_dead:", float(corr_dead) / float(dead_num))
    print("corr_all:", float(corr_all) / float(all_num))
    print("date_err:", float(date_error) / float(dead_num))
...
def view():
    ...
    ct_img, pet_img, date, live = data_reader.get_one(0)
...
def view_single(path, pet_id):
    live_net = LIVE_NET()
    live_net.cuda().eval()

    ckpt_path = config_livePredict['ckpt_path']
    data_path, step = livePredict_util.load_weight(ckpt_path)

    if step:
        live_net.load_state_dict(torch.load(data_path))

    # path = r"E:\petct肺癌\肺癌资料petct\18436~18435"
    # pet_id = "18436"
    infer_reader = INFER_READER(os.path.join(config_livePredict['extractfiles'], path))
