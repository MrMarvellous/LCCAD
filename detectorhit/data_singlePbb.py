...
# create by cy date: 2019 08 03 only for test
class SinglePbbSet(Dataset):
    def __init__(self, split, config, phase='test'):
        ...
        idcs = split
        # self.filenames = [os.path.join(datadir, '%s_clean.npy' % idx.split('-')[0]) for idx in idcs] # origin data
        self.filenames = [os.path.join(datadir, '%s_clean.npy' % idx) for idx in idcs]  # test new data
        ...
        for idx in idcs:
            ...
            print('candidate box size:', len(self.candidate_box), '###################')
        self.crop = simpleCrop(config, phase)

    def __getitem__(self, idx, split=None):
        # idx: index in filename list
        # return: this filename, (single filename's pbb) index list, crop data list, coord data list
        t = time.time()
        ...
        topk=pbb_length  # or min(pbb_length,10)

        indexlist = np.zeros([topk,]).astype('float32')
        ...
        for index in range(topk):
            ...
            croplist[index] = crop
            coordlist[index] = coord

        clean_name = os.path.basename(self.filenames[idx]).split('_clean.npy')[0]


        return clean_name,torch.from_numpy(indexlist).float(),torch.from_numpy(croplist).float(), torch.from_numpy(coordlist).float()
